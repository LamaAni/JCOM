...
namespace JCOM.OOSerializer.Javascript
{
    public class JsonStringArrayDataProvider : JsonMemoryArrayDataProvider<string>
    {
        ...
        public override void UpdateSource(bool isPretty = 
#if DEBUG
            true
#else 
            false
        ...
        /// <summary>
        /// Clones the data provider to create a new clean data provider with the same source.
        /// </summary>
        /// <returns></returns>
        public JsonStringArrayDataProvider Clone()
        {
            UpdateSource();
            JsonStringArrayDataProvider provider =
                new JsonStringArrayDataProvider(this.RawSource, Serializer);
            return provider;
        }
    }
}
